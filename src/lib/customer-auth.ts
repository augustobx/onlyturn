...
export async function createCustomerSession(accountId:string,tenantId:string){
 ...
 (await cookies()).set(COOKIE_NAME,token,{httpOnly:true,sameSite:"lax",secure:process.env.COOKIE_SECURE==="true",path:"/",maxAge:SESSION_DAYS*86_400});
}
...
export async function requireCustomerSession(tenantId:string,slug:string){const session=await getCustomerSession(tenantId);if(!session)redirect(`/r/${slug}/cuenta`);return session}
