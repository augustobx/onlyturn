...
export async function registerCustomerAction(_:CustomerAuthState,formData:FormData):Promise<CustomerAuthState>{
 ...
 try{
  ...
  if(status==="ACTIVE"){await createCustomerSession(account.id,tenant.id);redirect(`/r/${tenant.slug}/mi-cuenta`)}
  redirect(`/r/${tenant.slug}/cuenta?registered=pending`);
...
export async function logoutCustomerAction(formData:FormData){const slug=z.string().parse(formData.get("slug"));await destroyCustomerSession();redirect(`/r/${slug}`)}
