...
export function BookingWizard({ slug, currency, timezone, cancellationHours, locations, services, packages, customer }: {
  ...
  if (done) return <div className="wizard success pwa-success"><div className="success-icon"><Check /></div><span className="eyebrow">Reserva confirmada</span><h2>¡Listo! Tu reserva quedó agendada.</h2><p className="muted">Ya podés verla y administrarla desde Mi cuenta.</p><a className="button" href="/mi-cuenta">Ver mis turnos</a></div>;
  ...
  return <div className="booking-flow pwa-booking-flow">
    ...
    <section className="booking-selection card">
      <div className="booking-section-title"><span className="selection-number">1</span><div><h2>¿Qué querés reservar?</h2><p className="muted">Elegí la sede y el servicio. OnlyTurn se ocupa de cruzar la disponibilidad.</p></div></div>
      {locations.length > 1 && <><span className="pwa-mini-title"><MapPin size={14}/> Sede</span><div className="choice-chips">{locations.map((item) => <button type="button" className={locationId === item.id ? "active" : ""} onClick={() => chooseLocation(item.id)} key={item.id}><strong>{item.name}</strong><small>{item.address || "Atención disponible"}</small></button>)}</div></>}
      {locations.length === 1 && <div className="single-location"><MapPin size={15}/><span><strong>{locations[0].name}</strong>{locations[0].address && <small>{locations[0].address}</small>}</span></div>}
      <div className={`service-cards ${locationId ? "" : "locked-choice"}`} ref={serviceRef}>
        ...
        {categories.flatMap((category) => locationServices.filter((item) => (item.category || "General") === category).map((item) => {
          ...
          return <button type="button" className={serviceId === item.id ? "active" : ""} onClick={() => chooseService(item.id)} key={item.id}><span className="service-color" style={{ background: item.color }} /><span><small>{category} · {bookingTypeLabel[item.bookingType]}</small><strong>{item.name}</strong><small>{item.durationMinutes} min{item.maxPartySize > 1 ? ` · hasta ${item.maxPartySize} personas` : ""}</small>{item.description && <small>{item.description}</small>}{policy.enabled && <em>{policy.mode === "FULL" ? "Pago online" : `Seña ${policy.percent ?? 30}%`}</em>}</span><b>{item.priceCents == null ? "" : money.format(item.priceCents / 100)}</b></button>;
        }))}
      </div>

      {service && !isSessionType && (service.professionalMode !== "NONE" || service.resourceMode !== "NONE") && <div className="assignment-row"><div className="assignment-intro"><UserRound size={16}/><span><strong>Preferencia de atención</strong><small>Si no elegís una preferencia, OnlyTurn puede asignar automáticamente cuando el servicio lo permite.</small></span></div>{service.professionalMode !== "NONE" && service.professionals.length > 0 && <div className="field"><label>{service.professionalMode === "REQUIRED" ? "Profesional *" : "Profesional"}</label><select className="select" value={professionalId} onChange={(event) => { setProfessional(event.target.value); setSlot(""); }}>{service.professionalMode !== "REQUIRED" && <option value="">Cualquiera disponible</option>}{service.professionals.map((item) => <option value={item.professional.id} key={item.professional.id}>{item.professional.name}</option>)}</select></div>}{service.resourceMode !== "NONE" && service.resources.length > 0 && <div className="field"><label>{service.resourceMode === "REQUIRED" ? "Recurso *" : "Recurso"}</label><select className="select" value={resourceId} onChange={(event) => { setResource(event.target.value); setSlot(""); }}>{service.resourceMode !== "REQUIRED" && <option value="">Cualquiera disponible</option>}{service.resources.map((item) => <option value={item.resource.id} key={item.resource.id}>{item.resource.name}{item.resource.type ? ` · ${item.resource.type}` : ""}</option>)}</select></div>}</div>}

      {service?.addons.length ? <div className="pwa-addons"><div className="section-head"><div><span className="pwa-mini-title"><Sparkles size={14}/> Extras</span><h3>¿Querés agregar algo?</h3></div><span className="muted">Opcional</span></div><div className="option-grid">{service.addons.map((addon) => { const selected = addonIds.includes(addon.id); return <label className={`option ${selected ? "active" : ""}`} key={addon.id}><span><strong>{addon.name}</strong>{addon.description && <small>{addon.description}</small>}<small>{addon.durationMinutes ? `+${addon.durationMinutes} min · ` : ""}{money.format(addon.priceCents / 100)}</small></span><input type="checkbox" checked={selected} onChange={() => toggleAddon(addon.id)} /></label>; })}</div>{selectedAddons.length > 0 && <div className="addon-summary">Extras seleccionados: <strong>{money.format(addonPriceCents / 100)}</strong>{!isSessionType && addonDuration ? ` · +${addonDuration} min` : ""}</div>}</div> : null}
    </section>

    <section className={`booking-time card guided-section ${service ? "ready" : "locked"}`} ref={timeRef}>
      <div className="booking-section-title"><span className="selection-number">2</span><div><h2>{isSessionType ? "Elegí una fecha" : "¿Cuándo te queda bien?"}</h2><p className="muted">{isSessionType ? "Mostramos sólo sesiones con cupo real." : "Los horarios que ves ya están libres; no necesitás comprobar nada más."}</p></div></div>
      {isSessionType ? <>
        ...
        <div className="date-navigation"><button type="button" disabled={!service} onClick={() => setWeekStart(addUtcDays(weekStart, -7))} aria-label="Semana anterior"><ChevronLeft/></button><div className="date-strip">{days.map((item) => { const value = new Date(`${item}T12:00:00Z`); return <button type="button" disabled={!service || item < initialDate} className={date === item ? "active" : ""} onClick={() => { setDate(item); setSlot(""); }} key={item}><small>{new Intl.DateTimeFormat("es-AR", { weekday: "short", timeZone: "UTC" }).format(value)}</small><strong>{value.getUTCDate()}</strong><span>{new Intl.DateTimeFormat("es-AR", { month: "short", timeZone: "UTC" }).format(value)}</span></button>; })}</div><button type="button" disabled={!service} onClick={() => setWeekStart(addUtcDays(weekStart, 7))} aria-label="Semana siguiente"><ChevronRight/></button></div>
        <div className="selected-day-line"><CalendarDays size={15}/><strong>{dateLabel(date)}</strong><input type="date" value={date} min={initialDate} onChange={(event) => { setDate(event.target.value); setWeekStart(event.target.value); setSlot(""); }} /></div>
        {loadingAvailability ? <AvailabilityLoading /> : slots.length > 0 ? <div className="time-periods">{slotGroups.map((group) => <section key={group.key}><div className="time-period-title"><span>{group.label}</span><small>{group.slots.length} horario{group.slots.length === 1 ? "" : "s"}</small></div><div className="time-groups">{group.slots.map((value) => <button type="button" className={slot === value ? "active" : ""} onClick={() => chooseSlot(value)} key={value}>{timeLabel(value, timezone)}</button>)}</div></section>)}</div> : assignmentReady ? <div className="no-slot-state"><span className="no-slot-icon"><CalendarDays/></span><h3>No hay lugar el {dateLabel(date, "short")}</h3><p>Buscamos automáticamente las próximas fechas disponibles para este servicio.</p>{nextAvailability.length > 0 ? <div className="next-availability"><span className="pwa-mini-title"><Sparkles size={14}/> Próximos disponibles</span>{nextAvailability.map((day) => <button type="button" onClick={() => chooseNextDay(day.date)} key={day.date}><span><strong>{dateLabel(day.date)}</strong><small>{day.slots.length} horario{day.slots.length === 1 ? "" : "s"} · desde {timeLabel(day.slots[0], timezone)}</small></span><ChevronRight size={17}/></button>)}</div> : <small className="muted">No encontramos disponibilidad cercana. Probá otra semana o anotate en lista de espera.</small>}</div> : <div className="empty">Completá las selecciones requeridas para consultar disponibilidad.</div>}
        {!loadingAvailability && assignmentReady && !slots.length && service?.allowWaitlist && <WaitlistForm slug={slug} locationId={locationId} serviceId={service.id} professionalId={professionalId || undefined} resourceId={resourceId || undefined} preferredDate={date} minPartySize={service.minPartySize} maxPartySize={service.maxPartySize} customer={customer} />}
      </>}
    </section>

    <section className={`booking-details card guided-section ${selectionReady ? "ready" : "locked"}`} ref={detailsRef}>
      <div className="booking-section-title"><span className="selection-number">3</span><div><h2>Confirmá tu reserva</h2><p className="muted">{selectedDate ? new Intl.DateTimeFormat("es-AR", { dateStyle: "full", timeStyle: "short", timeZone: timezone }).format(new Date(selectedDate)) : "Primero elegí una fecha y horario"}</p></div></div>
      {selectionReady && service && <form onSubmit={submit}>
        {(service.maxPartySize > 1 || isSessionType) && <div className="field"><label>Cantidad de asistentes</label><input className="input" name="partySize" type="number" min={service.minPartySize} max={Math.max(service.minPartySize, partyMax)} value={partySize} onChange={(event) => updatePartySize(Number(event.target.value))} required /><small className="muted">Hasta {Math.max(service.minPartySize, partyMax)} para esta reserva.</small></div>}
        {eligiblePackages.length > 0 && <div className="field"><label>Paquete / membresía</label><select className="select" value={customerPackageId} onChange={(event) => setCustomerPackageId(event.target.value)}><option value="">No usar paquete</option>{eligiblePackages.map((membership) => <option value={membership.id} key={membership.id}>{membership.name} · {membership.remainingUses} uso{membership.remainingUses === 1 ? "" : "s"}</option>)}</select></div>}
        <div className="customer-grid"><div className="field"><label>Nombre *</label><input className="input" name="firstName" autoComplete="given-name" defaultValue={customer?.firstName} required /></div><div className="field"><label>Apellido</label><input className="input" name="lastName" autoComplete="family-name" defaultValue={customer?.lastName} /></div><div className="field"><label>Teléfono *</label><input className="input" name="phone" type="tel" autoComplete="tel" defaultValue={customer?.phone} required /></div><div className="field"><label>Email</label><input className="input" name="email" type="email" autoComplete="email" defaultValue={customer?.email} required={Boolean(paymentPolicy.enabled && payablePriceCents > 0)} /></div></div>
        ...
        {(service.priceCents != null || addonPriceCents > 0) && <div className="payment-callout"><strong>{selectedPackage ? `Usando ${selectedPackage.name}` : "Total"}</strong><span>{selectedPackage ? `${partySize} uso${partySize === 1 ? "" : "s"}` : money.format(totalPriceCents / 100)}</span>{selectedPackage && <small>{addonPriceCents > 0 ? `Extras a pagar: ${money.format(addonPriceCents / 100)}.` : "El servicio queda cubierto por tu membresía."}</small>}</div>}
        {paymentPolicy.enabled && payablePriceCents > 0 && <div className="payment-callout payment-online"><strong>{paymentPolicy.mode === "FULL" ? "Pago para confirmar" : "Seña para confirmar"}</strong><span>{money.format(paymentTotal)}</span><small>Al confirmar te llevamos a Mercado Pago.</small></div>}
        <button className="button confirm-booking" disabled={pending}>{pending ? "Confirmando tu reserva…" : paymentPolicy.enabled && payablePriceCents > 0 ? "Reservar y pagar" : selectedPackage ? `Confirmar con ${selectedPackage.name}` : "Confirmar reserva"}</button>
      </form>}
    </section>
  </div>;
}
